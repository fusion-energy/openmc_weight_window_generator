...
def magic(model, tally, iterations, rtol=0.7):
    """
    ...
    tally : openmc.Tally
        The tally to use for weight window generation
    iterations : int
        The number of iterations to perform
    rtol : float (default: 0.7)
        Upper limit on relative error of flux values used to produce
        weight windows.
    """

    # determine the name of the statepoint file that will be generated by a run
    ...
    ww_gen = update_wws(model, statepoint_name, tally, rtol)

    for i in range(iterations):
        _magic_inner(ww_gen)

    settings_with_ww = openmc.Settings.from_xml('settings.xml')
    return settings_with_ww.weight_windows[0]
...
def update_wws(model, statepoint_name, tally, rtol):

    # get some information about the tally
    tally, mesh_filter, e_bounds, particles = _get_tally_info(statepoint_name, tally)
    ...
    while(True):
        ...
        tally_result, mesh_filter, e_bounds, particles = _get_tally_info(statepoint_name, tally)

        wws = []
        for particle in particles:
            # get_value_args = (['flux'], [openmc.ParticleFilter], [particle])
            # flux_mean = tally_result.get_values(*get_value_args)
            flux_mean = tally_result.get_values(scores=['flux'])
            ...
            if N > 1:
                ...
                mean = flux_mean
                # rel_err = tally_result.get_values(*get_value_args, value='rel_err').reshape(sum[particle].shape)
                rel_err = tally_result.get_values(scores=['flux'], value='rel_err').reshape(sum[particle].shape)

            # filter out values with high relative error
            filter_indices = np.logical_and(rel_err > rtol, np.isfinite(rel_err))
...
def _get_tally_info(statepoint_name, tally):
    ...
    with openmc.StatePoint(statepoint_name) as sp:
        ...
        try:
            ef = tally_result.find_filter(openmc.EnergyFilter)
        ...
        if 'flux' not in tally_result.scores:
            msg = (
                'The MAGIC method requires a flux score on the tally. '
                f'No flux score is present on tally with ID {tally.id}'
            )
        ...
        return tally_result, mesh_filter, e_groups, particles
